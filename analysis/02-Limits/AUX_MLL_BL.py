    ...
    for its in range(num_pseudo):
        ...
        if D_or_E == 'exclusion': # only Background events
            ...
            if min(N_pseudo) >= MIN_EVS:
                ...
                # reference points
                sum_muhat_zero = sum ( [((ni*si) / (((-0.01)*si) + bi)) - si for ni, si, bi in zip(N_pseudo, sig_prom_bins, back_prom_bins)] )
...
def MLL_test_fast(D_or_E, pB_B_data, pS_B_data, pB_S_data, pS_S_data, B_expected, S_expected, num_pseudo):
    ...
    for its in range(num_pseudo):
        ...
        if D_or_E == 'exclusion': # only Background events
            ...
            # reference points
            sum_muhat_zero = sum ( [(x*1.) / ( (x * (-0.05) * S_expected) + y ) for x, y in zip(prob_x_given_S, B_prob_x_given_B)] )
